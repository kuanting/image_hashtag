...
def command():
    ...
    parser.add_argument("-m", "--mode", type=str, dest="mode", help="train or test, Default is train", default="train", choices=['train', 'test'])
    ...
    parser.add_argument("-s", "--save", type=str, dest="save", help="Path of training model, default is model/", default="model/")
    ...
    parser.add_argument("-model_type", type=float, dest="mt", choices=['multi_label', 'devise', 'convex', 'hades'])
    return parser
...
def testing(img_extractor, img2vec, word2idx, word_vec_dict, vec_matrix):
    fix_path = "../Hashtag/HARRISON/instagram_dataset/"
...
if __name__ == "__main__":
    ...
    img2vec_model = img2vec().to(device)

    if args.mode == "train":
        if args.mt == "devise":
            img_extractor_model.train()
            img2vec_model.train()
            training(args, img_extractor_model, img2vec_model, folder_dir, word2idx, word_vec_dict, vec_matrix)
    else:
        if args.mt == "devise":
            m_path = os.path.join(args.save, 'best_val')
            img_extractor_model.load_state_dict(torch.load(m_path+'_img_extractor_model.pth'))
            img2vec_model.load_state_dict(torch.load(m_path+'_img2vec_model.pth'))
            img_extractor_model.eval()
            img2vec_model.eval()
            while True:
                testing(img_extractor_model, img2vec_model, word2idx, word_vec_dict, vec_matrix)
